...
class NoteViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        # видеть только свои заметки:
        # if self.request.user.admin:
        #     return self.model.objects.all()
        # return self.model.objects.filter(author=self.request.user)

        # видеть все заметки всех пользователей:
        return self.model.objects.all()
...
class NoteViewSet(ModelViewSet):
    ...
    queryset = model.objects.none()
    serializer_class = NoteSerializer
    # http_method_names = ['get', 'post']  # можете указать только те методы, которые будут доступны
...
class NoteListView(ListModelMixin, CreateModelMixin, GenericAPIView):
    queryset = Note.objects.all()  # нужное поле для Generic, Mixin
    serializer_class = NoteSerializer  # нужное поле для Generic, Mixin
